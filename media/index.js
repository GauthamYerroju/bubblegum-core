...
class DefaultHandler {
  ...
  static inspect (file) {
    return Promise.reject({ code: 'ENOHANDLER', message: `Handler not found for: ${file}.` }) // eslint-disable-line prefer-promise-reject-errors
  }
...
class Media {
    ...
    for (const type of handler.handlesTypes) {
      if (!(type in this.typeMap)) {
        ...
        if (handler.name in this.typeMap[type].available) {
          throw Error(`${handler.name} is already registered for ${type}.`)
        } else {
  ...
  static setHandler (type, handler) {
    if (!(handler.name in this.nameMap)) {
      throw Error(`${handler.name} is not registered.`)
    }
